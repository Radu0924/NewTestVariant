...
class RingBuffer:
    ...
    def write(self, data: np.ndarray) -> int:
        ...
        with self._not_full:
            # If incoming block is larger than buffer, keep only the tail
            if samples >= self._buffer_size:
                data = data[:, -self._buffer_size:]
                self._buffer[:, :] = data
                self._read_pos = 0
                self._write_pos = 0
                self._available = self._buffer_size
                self._not_empty.notify_all()
                return self._buffer_size

            # Drop oldest samples if needed to make room
            free = self._buffer_size - self._available
            if samples > free:
                drop = samples - free
                self._read_pos = (self._read_pos + drop) % self._buffer_size
    ...
    def read(self, samples: int, timeout: float = 1.0) -> Optional[np.ndarray]:
        ...
        if samples > self._buffer_size:
            raise ValueError(
                f"Requested {samples} samples, but buffer holds {self._buffer_size}"
            )
...
class AudioCapture:
    ...
    def __init__(
        ...
        Args:
            num_channels: Number of input channels (>= 8).
            sample_rate: Sample rate in Hz.
            buffer_size: Buffer size in samples.
            device_index: Audio device index (None for default).
            buffer_seconds: Ring buffer duration in seconds.
            requested_channels: Requested device input channels (None uses num_channels).
            enabled_channels: List of enabled channel indices (>= 8).
            channel_map: Optional device channel map (overrides enabled_channels).
        """
        self._requested_channels = requested_channels if requested_channels is not None else num_channels
        ...
        if channel_map is not None:
            enabled_device_channels = list(channel_map)
            self._enabled_channels_user_set = True
        elif enabled_channels is not None:
            enabled_device_channels = list(enabled_channels)
            self._enabled_channels_user_set = True
        else:
            enabled_device_channels = list(range(self._requested_channels))
            self._enabled_channels_user_set = False

        self._enabled_device_channels: List[int] = enabled_device_channels
        self._enabled_count = len(enabled_device_channels)
        if self._enabled_count < 8:
            raise ValueError("At least 8 enabled channels are required")

        self._num_channels = self._enabled_count

        self._ring_buffer = RingBuffer(
            self._enabled_count, buffer_seconds, sample_rate
        )
        ...
        self._channel_status: List[ChannelStatus] = [
            ChannelStatus(i) for i in range(self._enabled_count)
        ]
    ...
    def _validate_channel_selection(
        ...
        enabled_device_channels: List[int]
    ) -> None:
        """Validate enabled channel selection against requested channels."""
        if requested_channels <= 0:
            raise ValueError("Requested channels must be positive")

        if len(enabled_device_channels) < 8:
            raise ValueError("At least 8 enabled channels are required")

        seen = set()
        for idx in enabled_device_channels:
            if not isinstance(idx, int):
                raise ValueError("Channel indices must be integers")
            if idx < 0 or idx >= requested_channels:
                raise ValueError(
                    f"Enabled channel index {idx} is out of range for "
                    f"requested_channels={requested_channels}"
                )
            if idx in seen:
                raise ValueError(f"Duplicate channel index detected: {idx}")
            seen.add(idx)
    ...
    def _audio_callback(
        ...
        # Select enabled channels, convert to float32 and transpose
        selected = indata[:, self._enabled_device_channels]
        data = selected.T.astype(np.float32)
    ...
    def start(self) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                self._actual_channels = requested

                if not self._enabled_channels_user_set:
                    self._enabled_device_channels = list(range(requested))
                ...
                self._enabled_count = len(self._enabled_device_channels)
                self._num_channels = self._enabled_count
                self._ring_buffer = RingBuffer(
                    self._enabled_count, self._buffer_seconds, self._sample_rate
                )
    ...
    @property
    def enabled_channels_count(self) -> int:
        """Get number of enabled channels."""
        return self._enabled_count

    @property
    def enabled_device_channels(self) -> List[int]:
        """Get enabled device channel indices (ordered)."""
        return self._enabled_device_channels.copy()
    ...
    def statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'actual_channels': self._actual_channels,
            'enabled_channels_count': self._enabled_count,
            'enabled_device_channels': self._enabled_device_channels,
