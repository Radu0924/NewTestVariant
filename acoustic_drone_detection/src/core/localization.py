...
class LocalizationConfig:
    ...
    robust_loss: str = "huber"  # linear, huber, soft_l1, cauchy, arctan
    f_scale: float = 1.0
...
class MultilaterationSolver:
    ...
    def localize(
        ...
        Args:
            ...
            weights: Optional confidence weights per channel (same length as tdoas).
        ...
        # Prepare weights (ignore reference channel at index 0)
        sqrt_w = None
        if weights is not None and len(weights) >= len(tdoas):
            w = np.clip(np.asarray(weights[1:]), 1e-6, 1.0)
            sqrt_w = np.sqrt(w)

        def residual_fn(position: np.ndarray) -> np.ndarray:
            residuals = self._residual_function(position, range_diffs)
            if sqrt_w is not None:
                return residuals * sqrt_w
            return residuals

        loss = self._config.robust_loss
        if loss not in ["linear", "huber", "soft_l1", "cauchy", "arctan"]:
            loss = "linear"
        ...
        if sqrt_w is not None:
            confidence *= float(np.mean(np.clip(weights[1:], 0.0, 1.0)))
        ...
        try:
            if result.jac is not None:
                ...
                res = result.fun
                dof = max(1, len(res) - 3)
                sigma2 = float(np.dot(res, res) / dof)
                covariance = np.linalg.pinv(JTJ) * sigma2
        except Exception:
            covariance = None
...
class HybridLocalizer:
    ...
    def localize(
        ...
        tdoa_confidences: Optional[np.ndarray] = None
    ) -> LocalizationResult:
        """
        ...
        Args:
            ...
            tdoa_confidences: Optional confidence weights for TDOAs.
        ...
        if tdoas is not None and len(tdoas) >= 3:
            mlat_result = self._multilateration.localize(
                tdoas, weights=tdoa_confidences
            )
