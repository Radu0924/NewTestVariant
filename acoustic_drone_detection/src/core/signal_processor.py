...
class FilterBank:
    ...
    def _init_filters(self) -> None:
        ...
        self._state_channels = None
        self._bandpass_zi = None
        self._highpass_zi = None
        self._notch_zi = None

    def _ensure_state(self, num_channels: int) -> None:
        """Initialize or reset streaming filter states for channel count."""
        if self._state_channels == num_channels and self._bandpass_zi is not None:
            return

        self._state_channels = num_channels

        # Bandpass state per channel
        self._bandpass_zi = [
            sosfilt_zi(self._bandpass_sos) for _ in range(num_channels)
        ]

        # High-pass state per channel
        self._highpass_zi = [
            sosfilt_zi(self._highpass_sos) for _ in range(num_channels)
        ]

        # Notch state per channel, per notch filter
        self._notch_zi = []
        for b, a in self._notch_filters:
            zi = lfilter_zi(b, a)
            self._notch_zi.append(np.tile(zi, (num_channels, 1)))
    ...
    def apply_bandpass(self, data: np.ndarray) -> np.ndarray:
        ...
        if data.ndim == 1:
            ...
            filtered, zi = sosfilt(self._bandpass_sos, data, zi=self._bandpass_zi[0])
            self._bandpass_zi[0] = zi
            return filtered
        ...
        for ch in range(num_channels):
            result[ch], self._bandpass_zi[ch] = sosfilt(
                self._bandpass_sos,
                data[ch],
    ...
    def apply_notch(self, data: np.ndarray) -> np.ndarray:
        ...
        if data.ndim == 1:
            self._ensure_state(1)
            result = data.copy()
            for idx, (b, a) in enumerate(self._notch_filters):
                zi = self._notch_zi[idx][0]
                result, zi_new = lfilter(b, a, result, zi=zi)
                self._notch_zi[idx][0] = zi_new
            return result
        ...
        for idx, (b, a) in enumerate(self._notch_filters):
            for ch in range(num_channels):
                zi = self._notch_zi[idx][ch]
                result[ch], zi_new = lfilter(b, a, result[ch], zi=zi)
                self._notch_zi[idx][ch] = zi_new
    ...
    def apply_highpass(self, data: np.ndarray) -> np.ndarray:
        ...
        if data.ndim == 1:
            self._ensure_state(1)
            filtered, zi = sosfilt(self._highpass_sos, data, zi=self._highpass_zi[0])
            self._highpass_zi[0] = zi
            return filtered
        ...
        result = np.zeros_like(data)
        for ch in range(num_channels):
            result[ch], self._highpass_zi[ch] = sosfilt(
                self._highpass_sos,
                data[ch],
                zi=self._highpass_zi[ch]
            )
        return result
...
class SignalProcessor:
    ...
    def _ensure_channel_processors(self, num_channels: int) -> None:
        """Ensure per-channel AGC and noise gate instances exist."""
        if self._channel_count == num_channels and self._agc_list:
            return

        self._channel_count = num_channels
        self._noise_gate_list = [
            NoiseGate(sample_rate=self._sample_rate) for _ in range(num_channels)
        ]
        self._agc_list = [
            AGC(sample_rate=self._sample_rate) for _ in range(num_channels)
        ]
    ...
    def preprocess(
        ...
        if result.ndim > 1:
            self._ensure_channel_processors(result.shape[0])
            for i in range(result.shape[0]):
                if apply_noise_gate:
                    result[i] = self._noise_gate_list[i].process(result[i])
                if apply_agc:
                    result[i] = self._agc_list[i].process(result[i])
        else:
            self._ensure_channel_processors(1)
            if apply_noise_gate:
                result = self._noise_gate_list[0].process(result)
            if apply_agc:
                result = self._agc_list[0].process(result)
