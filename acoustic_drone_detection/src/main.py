...
class DroneDetectionSystem:
    ...
    def __init__(
        ...
        # Optional channel selection/mapping
        enabled_channels = getattr(config.audio, "enabled_channels", None)
        channel_map = getattr(config.audio, "channel_map", None)
        requested_channels = getattr(config.audio, "requested_channels", None)

        if enabled_channels is not None:
            enabled_indices = list(enabled_channels)
        elif channel_map is not None:
            # Channel map reorders device channels; use first N mics in geometry
            enabled_indices = list(range(len(channel_map)))
        else:
        ...
        if enabled_indices is not None:
            if max(enabled_indices) >= mic_positions.shape[0]:
                raise ValueError(
                    "Enabled channel index exceeds available microphone positions"
                )
            mic_positions = mic_positions[enabled_indices]
        ...
        tdoa_config = TDOAConfig()
        if mic_positions.shape[0] >= 2:
            diffs = mic_positions[:, None, :] - mic_positions[None, :, :]
            max_pair_distance = float(np.max(np.linalg.norm(diffs, axis=2)))
        else:
            max_pair_distance = 1.0
        tdoa_config.max_delay_meters = max_pair_distance
        self._tdoa_engine = TDOAEngine(
            sample_rate=config.audio.sample_rate,
            config=tdoa_config
        )
    ...
    def _process_frame(self, audio_data: np.ndarray) -> Optional[DetectionEvent]:
        ...
        # Localization (TDOA + MLAT when available)
        signal_rms = np.sqrt(np.mean(mono ** 2))
        loc_result = None

        try:
            if filtered.ndim > 1 and filtered.shape[0] >= 4:
                n_channels = filtered.shape[0]
                tdoas = np.zeros(n_channels, dtype=np.float64)
                tdoa_confidences = np.ones(n_channels, dtype=np.float64)
                ref = 0

                for ch in range(n_channels):
                    if ch == ref:
                        continue
                    res = self._tdoa_engine.estimate_tdoa(
                        filtered[ref],
                        filtered[ch],
                        ref,
                        ch
                    ...
                    tdoa_confidences[ch] = res.confidence

                min_conf = getattr(
                    self._config_manager.config.detection, 'tdoa_min_confidence', 0.15
                )
                valid_pairs = int(np.sum(tdoa_confidences[1:] >= min_conf))

                if valid_pairs >= 3:
                    loc_result = self._localizer.localize_hybrid(
                        doa_result.azimuth,
                        doa_result.elevation,
                        tdoas=tdoas,
                        signal_rms=signal_rms,
                        tdoa_confidences=tdoa_confidences
                    )
        except Exception as e:
            self._logger.error(f"TDOA/localization error: {e}")
